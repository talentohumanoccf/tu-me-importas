...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (testSheetsBtn) {
    testSheetsBtn.addEventListener('click', async () => {
      ...
      sheetsStatus.innerHTML = '⏳ Conectando con hoja BASE_PX...';
      try {
        const res = await fetch(url, { method: 'GET' });
        const data = await res.json();
        if (data.status === 'online') {
          sheetsStatus.innerHTML = `🟢 Conexión Exitosa a Google Sheets (Hoja activa: ${data.baseSheet || 'BASE_PX'})`;
          sheetsStatus.style.color = 'var(--success)';
        } else {
          sheetsStatus.innerHTML = '🟡 Conexión establecida pero con respuesta inesperada.';
        }
      } catch (err) {
        sheetsStatus.innerHTML = '🔴 Error al conectar. Verifique permisos en Google Apps Script (Anyone/Cualquier persona).';
        sheetsStatus.style.color = 'var(--danger)';
      }
    });
  }
